...
public void ConnectToDb(Database db, const char[] failure, any data)
{
    ...
    else
    {
        ...
        if (!SQL_FastQuery(g_Database, "CREATE TABLE IF NOT EXISTS `data` (`steamid` varchar(32) NOT NULL, `ip` varchar(16) NOT NULL, `cc` varchar(2) NOT NULL, `isp` varchar(128) NOT NULL, `first_connect` datetime NOT NULL, `last_connect` datetime NOT NULL, `times_connected` int NOT NULL DEFAULT 0, CONSTRAINT pk PRIMARY KEY (steamid, ip))"))
        {
...
public int HTTPRequestCompleted(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode)
{
    ...
    {
        LogError("Failed to get API answer, error: %d", eStatusCode);
    }
    else
    {
        int size;
...
{
    Format(g_Query, sizeof(g_Query), "INSERT INTO data (steamid, ip, cc, isp, first_connect) VALUES ('%s', '%s', '%s', '%s', NOW()) ON DUPLICATE KEY UPDATE steamid = '%s'", g_SteamId, g_Ip, g_CountryCode, g_Isp, g_SteamId);
...
public void OnClientDisconnect(int client)
{
    if (!IsFakeClient(client) && g_Database != null)
    {
        Format(g_Query, sizeof(g_Query), "UPDATE data SET last_connect = NOW(), times_connected = times_connected + 1 WHERE steamid = '%s' AND ip = '%s'", g_SteamId, g_Ip);
        if (!SQL_FastQuery(g_Database, g_Query))
        {
            SQL_GetError(g_Database, g_Error, sizeof(g_Error));
